...
3. **The codebase.** Does the existing code teach the wrong patterns? If 50 existing functions use pattern A and you want pattern B, the agent will default to A regardless of instructions. *Test:* ask the agent *why* it chose its approach. If it cites existing code, the codebase is teaching patterns your instruction is supposed to override — make the instruction explicit about the exception.
...
In practice, 80% of persistent failures resolve at Layer 1 or Layer 2. Practitioners who jump to "the model is wrong" usually haven't verified their instructions are unambiguous and conflict-free.
...
**Prevention:** incremental refactoring — extract one function, write characterization tests, refactor, verify, commit. Each step independently shippable. Covered in the Starting & Refactoring Projects chapter.
...
- Four-layer diagnosis: prompt → briefing doc → codebase → model. 80% of failures resolve at Layer 1 or 2.

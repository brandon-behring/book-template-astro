...
**Configuration layers.** The agent's behavior is not a single rule set — it is a stack. A briefing document at the project root (`CLAUDE.md` / `GEMINI.md` / `AGENTS.md`), optional scoped rules, tool-level permissions, and in some tools an output-style layer. These layers are not suggestions — they are the operating system through which the agent interprets your project.
...
| --- | --- | --- | --- |
| Project briefing file | `CLAUDE.md` | `GEMINI.md` | `AGENTS.md` |
...
| Plan-mode entry | Shift+Tab | `/plan` | `--dry-run` |
...
<SkillBox title="Treat the briefing file as the most leveraged document you own">
  Every CLI-agent re-reads its briefing file on every turn. Every token in that file has multiplicative leverage. Budget it aggressively: project architecture (30 lines), code conventions (30 lines), non-obvious constraints (20 lines), explicit failure-mode instructions (20 lines), hard bounds on what the agent must not do (20 lines). A 120-line briefing file is not skimpy — it is typical for a well-run project. Over 200 lines means you're paying rent on every prompt for context the agent didn't need. Refactor into path-scoped rules.
</SkillBox>
...
**Emerging: plan-mode as a primitive.** All three tools have shipped (or committed to) an explicit planning phase where the agent reads and proposes without writing. Claude's was first-class first; Gemini shipped theirs in v0.8 (March 2026); Codex added `--dry-run` around the same time. Expect this to become a full convergence within a year.

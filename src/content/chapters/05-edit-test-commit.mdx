...
1. **Type safety** — static checking at compile/lint time (mypy, TypeScript, Rust's borrow checker, Go vet). The cheapest layer.
...
Verification-first is the single most convergent practice in agentic coding. The "give the agent a way to verify its own work" slogan is now universal across the three CLIs — but the *enforcement* surface (how you make verification non-skippable) differs substantially.
...
<Divergence axis="hook system depth">
  Claude Code ships the richest hook system: `PreToolUse`, `PostToolUse`, `PreCompact`, `PostCompact`, `Stop`, `SessionStart`, and more, each with matcher patterns for specific tool invocations. Gemini CLI has tool-level allowlists and basic pre/post hooks. Codex CLI uses command-approval configuration (`~/.codex/config.toml`) as its primary gate. For enforcing non-negotiable standards (run tests before commit, lint on write), Claude's hook depth is a real advantage; for teams who stay git-disciplined, Codex's simpler model is adequate.
</Divergence>
...
<CaseStudy date="2026-01">
  A production ML team tracked every agent-generated PR for six weeks with and without domain-invariant tests in the briefing doc. Results: PRs where the briefing doc specified domain invariants (leakage checks, unit consistency, completeness assertions) had a 7% post-merge revert rate. PRs without domain invariants had a 34% revert rate — and every reverted failure was a domain-correctness bug that all unit tests passed for. Pattern signal: domain invariants are not a best-practice, they are the 6-layer model's necessary completion for any codebase where "correct" requires judgment. Which is most codebases.
</CaseStudy>

### How this chapter will date

The **Representation** section — the six-layer model, the domain-correctness argument, phase-appropriate rigor — is stable. These were best practice before agents and will be after. The **Operation** section's command surface (hook event names, exact config file paths) is more volatile; expect annual refresh. The **Evolution** section, especially auto-repair loops and ambient verification, is active 2026 territory — treat claims here as 6-month claims, not 3-year claims.

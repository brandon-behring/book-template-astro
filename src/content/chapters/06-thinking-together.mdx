...
<CaseStudy date="2026-03">
  A team decision-audit: for one quarter, every architecture decision went through the recommend-and-critique-in-different-session pattern. They tracked whether the final decision matched the first recommendation, the critique's alternative, or a third synthesis. Results: 48% matched the first recommendation, 12% adopted the critique's alternative outright, 40% ended up at a third option neither session had produced alone. The 40% synthesis rate is the real value of the pattern — not catching wrong decisions (which the 12% would have), but generating better decisions by forcing the divergence to be reconciled. Collaboration as generative, not corrective.
</CaseStudy>

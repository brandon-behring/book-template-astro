...
sources: []
description: "CLAUDE.md / GEMINI.md / AGENTS.md — the industry has converged on a top-level briefing file the agent re-reads on every turn. This chapter is about what goes in it, what doesn't, and how to structure it so every token has leverage."
draft: false
---
...
*A stateless agent needs a frame. It has no memory of your project, no training on your conventions, no sense of what the current phase is or why this repo is shaped the way it is. The briefing document is how you give it one — on every single turn, at the top of context, where attention is strongest. This chapter is about what goes in that document, what shouldn't, and how to structure it so every line earns its place.*

## Representation

The briefing document is the single highest-leverage artifact in your project. Not because it's magic — because of where it sits in the conversation. Every modern CLI-agent re-reads the briefing doc at the start of every turn and injects it into context *before* the current prompt. This means every token in the file is paid-for on every single interaction of every session, forever. Waste is not free; density is compounding.
...
| Reload command | auto on compact | `/memory reload` | restart |
...
The **Representation** section is stable — the briefing-doc pattern has converged and is not going anywhere. The **Operation** section's tool-specific table has moderate drift risk; file paths and import mechanisms are the most volatile rows. The **Evolution** section's emerging patterns (linting, team-shared conventions) are active 2026 territory and will be worth re-verifying in 6–12 months.

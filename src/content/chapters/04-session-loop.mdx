...
The corollary is the **two-failure rule**: after two failed corrections on the same issue, clear context and restart with a better prompt. The math is unforgiving — two rounds of correction generate ~2,500 tokens of noise; a re-written opening prompt costs ~200 tokens and usually produces a better result.
...
| Enter plan mode | `Shift+Tab` → plan | `/plan` (v0.8+) | `--dry-run` flag |
...
<SkillBox title="The correction-free prompt shape">
  Every prompt for non-trivial work should answer five questions: *What changes?* (specific files, specific behavior) *In what context?* (relevant state, current failures, connected code) *Under what constraints?* (performance, compatibility, files off-limits) *Verified how?* (exact command to run, expected output). A prompt missing any of these is an invitation to correction rounds. The more boring the prompt, the better the result — see Ch 3 for the full five-part template.
</SkillBox>
...
**Convergence: plan mode as first-class.** Plan mode was a Claude-first feature in 2025; Gemini shipped explicit plan mode in v0.8 (March 2026); Codex's `--dry-run` flag behaves equivalently for most workflows. Recommending "start in plan mode for unfamiliar code" is now tool-independent advice.
...
<CaseStudy date="2026-02">
  A team instrumented their Claude Code sessions to count loop iterations per task and correlate with outcome quality. Results after two months: tasks that took ≤2 iterations had a 4% post-commit revert rate; tasks at 3 iterations had 11%; tasks at 4+ iterations had 28%. The pattern was robust across task size and developer. They adopted an automatic `/clear` prompt after the third iteration — pure discipline, zero tooling — and the 4+ bucket dropped to single digits over the next month. Pattern signal: the two-failure rule is a quality-preservation rule, not just a context-hygiene one.
</CaseStudy>

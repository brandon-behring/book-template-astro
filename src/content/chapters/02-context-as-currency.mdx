...
4. **Persistence** — a top-level briefing file (CLAUDE.md / GEMINI.md / AGENTS.md) that survives compaction and is re-injected on every turn.
...
| Reload persistence | auto on compact | `/memory reload` | auto on launch |
...
<CaseStudy date="2026-02">
  When Gemini shipped the 1M-token window, teams immediately tried to use all of it. Community benchmarking showed quality degradation starting around 40% fill and ~1-in-4 retrievals failing at capacity. The effective high-quality range turned out to be ~200–500K tokens — well short of the 1M headline. The tool didn't deceive anyone; the mental model did. Bigger window ≠ bigger usable workspace.
</CaseStudy>
...
**The briefing document** (`CLAUDE.md` / `GEMINI.md` / `AGENTS.md`) holds project rules, architecture, conventions. It is re-injected on every turn, so every token in it has leverage. Anthropic recommends individual files under 200 lines, combined under ~500 lines; the same bound applies across the three tools by analogy — the file is a context tax you pay on every single prompt, so every line must earn its place.
...
<Convergence tools={['claude-code', 'gemini-cli', 'codex-cli']}>
  All three CLI agents treat context as a budgeted, decaying resource. Each ships an explicit compaction command that summarizes the conversation in place; each supports a top-level briefing document re-injected on every turn. Practices written today can safely assume these primitives exist.
</Convergence>
...
<Divergence axis="context window size">
  Claude Code: 200K (1M on Opus 4.6+) · Gemini CLI: 1M native · Codex CLI: 128K–200K depending on model. The tradeoff is not settled: larger windows admit more material but exhibit more attention-decay noise; smaller windows force better curation but require more frequent boundary management. Write practices that do not depend on a specific ceiling.
</Divergence>
...
<CaseStudy date="2026-03">
  Gemini's union-find compaction landed as a feature flag in v0.8. Practitioners who tested it reported asynchronous summaries completing in ~200ms off the critical path, versus ~1.5s for the legacy inline approach. Pattern signal: as compaction becomes more sophisticated, practitioners lean on it more aggressively — which paradoxically makes briefing-doc discipline *more* important, not less, since the compacted history becomes less recoverable.
</CaseStudy>

### How this chapter will date

The freshness stamp above says `Last verified 2026-04-17`. The **Representation** section is stable — context rot is a property of transformer attention, not any specific tool. The **Operation** section is moderately durable; command names can change but the primitives (observe / compact / clear / persist) are architectural. The **Evolution** section is the most volatile — this is a `feature-surface` concern in a chapter otherwise flagged `architectural-pattern`, and it will need re-verification quarterly.
